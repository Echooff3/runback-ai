...
import SlashPromptModal from './SlashPromptModal';

export default function SlashPromptsScreen() {
  const [prompts, setPrompts] = useState<SlashPrompt[]>(getSlashPrompts());
  ...
  const handleEdit = (prompt: SlashPrompt) => {
    setEditingPrompt(prompt);
    setIsModalOpen(true);
  };

  const handleSave = (prompt: SlashPrompt) => {
    saveSlashPrompt({
      ...prompt,
      updatedAt: new Date().toISOString(),
    });
    setPrompts(getSlashPrompts());
    setIsModalOpen(false);
    setEditingPrompt(null);
  };

  const handleDelete = (id: string) => {
    if (confirm('Are you sure you want to delete this slash prompt?')) {
      deleteSlashPrompt(id);
      setPrompts(getSlashPrompts());
    }
  };
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Slash Prompts</h1>
          </div>
          <button
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search slash prompts..."
        ...
        {filteredPrompts.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 dark:text-gray-400">
              {searchTerm ? 'No prompts found matching your search.' : 'No slash prompts yet. Create one to get started!'}
            </p>
          </div>
        ) : (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <code className="text-lg font-mono font-semibold text-indigo-600 dark:text-indigo-400">
                      {prompt.command}
                    </code>
                    {prompt.isDefault && (
              ...
              <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-3 mb-3">
                <p className="text-sm text-gray-700 dark:text-gray-300 font-mono whitespace-pre-wrap">
                  {prompt.template || <span className="italic text-gray-500">No template</span>}
                </p>
              </div>
                ...
                <div className="mb-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {prompt.variables.map((variable, index) => (
                      <span
                        key={index}
                        className="text-xs px-2 py-0.5 bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200 rounded-full font-mono"
                      >
                        {'{' + variable.name + '}'}
                      </span>
                    ))}
